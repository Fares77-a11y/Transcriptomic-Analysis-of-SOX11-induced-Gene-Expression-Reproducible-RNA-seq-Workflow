...
│   ├── 02_preprocessing_qc.nf          # Nextflow workflow for QC, trimming (TrimGalore), and summary (MultiQC)
│   ├── 03_deg_analysis.R               # Main R script for DESeq2 analysis of gene counts
│   ├── 04_visualization.R              # Custom R code for DEGs plots: volcano, heatmap
│   ├── 05_pca_clustering.R             # PCA, sample distance, cluster analysis scripts
├── data/
│   ├── raw/                            # Place raw FASTQ or Kallisto outputs here (not stored in repo)
│   ├── processed/                      # Gene/tpm count tables post-alignment
...
│   └── nextflow.config                 # Nextflow configuration details
├── results/
│   ├── deseq2/
│   │   ├── DESeq2_DEGs_results.csv         # All gene stats
│   │   └── DESeq2_DEGs_significant.csv     # Significant DEGs (padj < 0.05)
│   ├── figures/
│   │   ├── sample_distance_heatmap.png     # Cluster heatmap
│   │   ├── pca_plot.png                    # PCA by phenotype
│   │   ├── volcano_plot_standard.png       # Volcano plot (padj < 0.05, |log2FC| > 1)
│   │   ├── volcano_plot_labeled.png        # Labeled volcano (padj < 0.01, |log2FC| > 2)
│   │   └── heatmap_top100_degs.png         # Top 100 DEGs heatmap
│   └── tables/
├── docs/
│   ├── methods.md
...
|--------|------|--------------|
| Sample Distance Heatmap | sample_distance_heatmap.png | VST-normalized, shows Euclidean clusters |
| PCA Plot | pca_plot.png | Visualizes main variance between conditions |
| Volcano Plot | volcano_plot_standard.png | All DEGs (padj<0.05, |log2FC|>1) |
| Labeled Volcano | volcano_plot_labeled.png | Key DEGs labeled (padj<0.01, |log2FC|>2) |
| Top 100 DEGs Heatmap | heatmap_top100_degs.png | Heatmap, top significant genes |

All figures are generated by the corresponding scripts and saved in `results/figures/`.

## References
- See manuscript for full bibliography. Major tools (with versions): Nextflow, nf-core, Cutadapt, FastQC, MultiQC, Kallisto, DESeq2, pheatmap, ggplot2, EnhancedVolcano. NCBI BioProject PRJNA1097853. See `docs/` for full citations.
